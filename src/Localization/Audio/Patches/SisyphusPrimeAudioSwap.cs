...
public class SisyphusPrimeAudioSwap
{
    ...
    private static void ApplyAudioSwap(SisyphusPrime __instance)
    {
        ...
        string folder = Path.Combine(AudioSwapper.SpeechFolder, "sisyphusPrime");

        AudioClip[] begoneAttacks = __instance.clapVoice;
        for (int i = 0; i < begoneAttacks.Length; i++)
            begoneAttacks[i] = AudioSwapper.SwapClipWithFile(begoneAttacks[i], Path.Combine(folder, "sisyphusBegone" + (i + 1)));

        AudioClip[] thisWillHurtAttack = __instance.explosionVoice;
        for (int i = 0; i < thisWillHurtAttack.Length; i++)
            thisWillHurtAttack[i] = AudioSwapper.SwapClipWithFile(thisWillHurtAttack[i], Path.Combine(folder, "sisyphusThisWillHurt"));

        AudioClip[] grunt = __instance.hurtVoice;
        for (int i = 0; i < grunt.Length; i++)
            grunt[i] = AudioSwapper.SwapClipWithFile(grunt[i], Path.Combine(folder, "sisyphusGrunt"));

        AudioClip[] stompAttacks = __instance.stompComboVoice;
        for (int i = 0; i < stompAttacks.Length; i++)
            stompAttacks[i] = AudioSwapper.SwapClipWithFile(stompAttacks[i], Path.Combine(folder, "sisyphusYouCantEscape" + (i + 1)));

        AudioClip[] taunts = __instance.tauntVoice;
        for (int i = 0; i < taunts.Length; i++)
            taunts[i] = AudioSwapper.SwapClipWithFile(taunts[i], Path.Combine(folder, "sisyphusNiceTry" + (i + 1)));

        AudioClip[] uppercutAttacks = __instance.uppercutComboVoice;
        for (int i = 0; i < uppercutAttacks.Length; i++)
            uppercutAttacks[i] = AudioSwapper.SwapClipWithFile(uppercutAttacks[i], Path.Combine(folder, "sisyphusDestroy" + (i + 1)));

        //Phase change
        __instance.phaseChangeVoice = AudioSwapper.SwapClipWithFile(__instance.phaseChangeVoice, Path.Combine(folder, "sisyphusYesThatsIt"));
    }
}
